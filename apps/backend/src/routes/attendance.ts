...
import { appendTransactionLog } from "../services/transactionLog.js";
  ...
  try {
    ...
    if (!year || !cls || !date || !period || !Array.isArray(absentRolls)) {
      await appendTransactionLog({
        ...
        date: date ?? "",
        period: period ?? "",
        absentRolls: Array.isArray(absentRolls) ? absentRolls : [],
        status: "REJECTED",
    ...
    if (!col) {
      await appendTransactionLog({
        ...
        absentRolls,
        status: "REJECTED",
        message: "Date/period not found in sheet",
      });

      return res.status(400).json({
        success: false,
        error: "Date/period not found in sheet — week may not be set up yet",
      });
    }
    ...
    const validRolls = new Set(
      rolls.map((r: any) => Number(r[0])).filter(Boolean)
    );
    ...
    for (const roll of uniqueAbsent) {
      if (!validRolls.has(roll)) {
        await appendTransactionLog({
          ...
          date,
          period,
          absentRolls: uniqueAbsent,
          status: "REJECTED",
          message: `Roll ${roll} not in master list`,
        });

        return res.status(400).json({
          success: false,
          error: `Roll ${roll} not in master list`,
        });
      }
    }
    ...
    if (existing.some((row: any) => row[0] === "P" || row[0] === "A")) {
      await appendTransactionLog({
        ...
        absentRolls: uniqueAbsent,
        status: "REJECTED",
        message: `${period} already marked for ${date}`,
      });

      return res.status(409).json({
        success: false,
        error: `${period} already marked for ${date}`,
      });
    }
